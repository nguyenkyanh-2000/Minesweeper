...
public class Board {
    ...
    /**
     * Interaction with cells on the board
     */

    public void firstLeftClick(int x, int y){
    ...
    public void useLeftClick(int x, int y) {
        Cell tmp = CellCollection.get(Cell.to1dIndex(x, y));
        if (tmp.isRevealed) {
    ...
    public void useRightClick(int x, int y){
        Cell tmp = CellCollection.get(Cell.to1dIndex(x,y));
        tmp.useRightClick();
        if (tmp.isFlagged == false){
            minesLeft--;
    ...
    public static List<Cell> getMineList() {
        return MineList;
    }
}
